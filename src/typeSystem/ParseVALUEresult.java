...
{ private final TypeT res;
  ...
  { str=str.trim();
    if(T.isUNIT())
    { if(str.startsWith("unit")) return ok(new TypeUnit(T), cutoff(str, "unit"));
      else{ return error(str, "Expected a UNIT TYPE value.");}
    }    
    else if(T.isBOOL())
    { if(str.startsWith("true")){ return ok(new TypeBool(T,(boolean)true), cutoff(str, "true"));}
      else if(str.startsWith("false")){ return ok(new TypeBool(T,(boolean)false), cutoff(str, "false"));}
      else{ return error(str, "Expected a BOOL TYPE value.");}
    }
    else if(T.isNAT())
    { if(startsWithInt(str)){ return ok(new TypeNat(T,Integer.parseInt(String.valueOf(getInt(str)))),cutoff(str, String.valueOf(getInt(str))));}
      else{ return error(str, "Expected a NAT TYPE value.");}
    }  
    else if(T.isREAL())
    { return ok(new TypeReal(T,getDouble(str)),cutoff(str,String.valueOf(getDouble(str))));  
    }    
    else if(T.isCHAR())
    ...
    {   if(!str.startsWith("[")){ return error(str, "Expected a LIST TYPE value.");}
      str=cutoff(str,"[");
        if(str.startsWith("]")){ str=cutoff(str,"]"); return ok(TypeList.EMPTY_LIST(T.getBaseTYPE()),str);}
